...
from fastapi import APIRouter, status, HTTPException, Depends, UploadFile, Query, BackgroundTasks
...
async def create_user(user: models.UserCreate, db: Annotated[AsyncSession, Depends(database.get_db)]):
    ...
    results = await db.execute(
        select(models.User).where(func.lower(col(models.User.email)) == user.email.lower()),
    )
...
@router.get("/me", response_model=models.UserPrivate)
    ...
    if user:
        await db.execute(
            sql_delete(models.PasswordResetToken).where(
                models.PasswordResetToken.user_id == user.id # type: ignore
            ),
        )
        ...
        reset_token = models.PasswordResetToken( # type: ignore
...
async def change_password(
    ...
    await db.execute(
        sql_delete(models.PasswordResetToken).where(
            models.PasswordResetToken.user_id == current_user.id, # type: ignore
        ),
    )
...
async def update_user(user_id: int, user_update: models.UserUpdate, current_user: CurrentUser, db: Annotated[AsyncSession, Depends(database.get_db)]):
    ...
    if user_update.username is not None and user_update.username.lower() != user.username.lower():
        result = await db.execute(select(models.User).where(func.lower(col(models.User.username)) == user_update.username.lower()))
    ...
    if user_update.email is not None and user_update.email.lower() != user.email.lower():
        result = await db.execute(select(models.User).where(func.lower(col(models.User.email)) == user_update.email.lower()))
...
@router.delete("/{user_id}/picture", response_model=models.UserPrivate)
