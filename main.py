...
async def user_posts_page(
    ...
    results = await db.execute(select(models.Post).options(selectinload(models.Post.author)).where(col(models.Post.user_id) == user_id))
    posts = results.scalars().all()
...
async def update_user(user_id: int, user_update: models.UserUpdate, db: Annotated[AsyncSession, Depends(models.get_db)]):
    result = await db.execute(
        select(models.User).where(col(models.User.id) == user_id),
    )
    user = result.scalars().first()
    ...
    if user_update.username is not None and user_update.username != user.username:
        result = await db.execute(select(models.User).where(col(models.User.username) == user_update.username))
    ...
    if user_update.email is not None and user_update.email != user.email:
        result = await db.execute(select(models.User).where(col(models.User.email) == user_update.email))
...
async def delete_user(user_id: int, db: Annotated[AsyncSession, Depends(models.get_db)]):
    result = await db.execute(select(models.User).where(col(models.User.id) == user_id))
    user = result.scalars().first()
    if not user:
        raise HTTPException(
...
async def get_user_posts(user_id: int, db: Annotated[AsyncSession, Depends(models.get_db)]):
    ...
    result = await db.execute(select(models.Post).options(selectinload(models.Post.author)).where(col(models.Post.user_id) == user_id))
...
async def create_post(post: models.PostCreate, db: Annotated[AsyncSession, Depends(models.get_db)]):
    result = await db.execute(select(models.User).where(col(models.User.id) == post.user_id))
    user = result.scalars().first()
    if not user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def update_post_full(post_id: int, post_data: models.PostCreate, db: Annotated[AsyncSession, Depends(models.get_db)]):
    result = await db.execute(select(models.Post).where(col(models.Post.id) == post_id))
    post = result.scalars().first()
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Post not found")
    if post_data.user_id != post.user_id:
        result = await db.execute(select(models.User).where(col(models.User.id) == post_data.user_id))
        user = result.scalars().first()
        if not user:
            raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
...
async def update_post_partial(post_id: int, post_data: models.PostUpdate, db: Annotated[AsyncSession, Depends(models.get_db)]):
    result = await db.execute(select(models.Post).where(col(models.Post.id) == post_id))
    post = result.scalars().first()
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Post not found")
...
async def delete_post(post_id: int, db: Annotated[AsyncSession, Depends(models.get_db)]):
    result = await db.execute(select(models.Post).where(col(models.Post.id) == post_id))
    post = result.scalars().first()
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Post not found")
